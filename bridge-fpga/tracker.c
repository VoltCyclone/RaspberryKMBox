 ...
 *
 * Fixed-point math for the hot pixel-scanning path.
 * Shared with the standard bridge implementation.
 */
...
void tracker_process_frame(const uint8_t* rgb, uint16_t w, uint16_t h, tracker_result_t* result) {
    ...
    // Apply gain (fixed-point)
    int32_t raw_dx_fp = (int32_t)cx * gain_x_fp;
    int32_t raw_dy_fp = (int32_t)cy * gain_y_fp;

    // EMA smoothing
    int32_t hi; uint32_t lo;
    __asm__ volatile ("smull %0, %1, %2, %3" : "=r" (lo), "=r" (hi) : "r" (smooth_dx_fp), "r" (smoothing_factor_fp));
    int32_t s_old_x = (int32_t)((uint32_t)(hi << TRACKER_FP_SHIFT) | (lo >> TRACKER_FP_SHIFT));
    __asm__ volatile ("smull %0, %1, %2, %3" : "=r" (lo), "=r" (hi) : "r" (raw_dx_fp), "r" (one_minus_smoothing_fp));
    int32_t s_new_x = (int32_t)((uint32_t)(hi << TRACKER_FP_SHIFT) | (lo >> TRACKER_FP_SHIFT));
    smooth_dx_fp = s_old_x + s_new_x;

    __asm__ volatile ("smull %0, %1, %2, %3" : "=r" (lo), "=r" (hi) : "r" (smooth_dy_fp), "r" (smoothing_factor_fp));
    int32_t s_old_y = (int32_t)((uint32_t)(hi << TRACKER_FP_SHIFT) | (lo >> TRACKER_FP_SHIFT));
    __asm__ volatile ("smull %0, %1, %2, %3" : "=r" (lo), "=r" (hi) : "r" (raw_dy_fp), "r" (one_minus_smoothing_fp));
    int32_t s_new_y = (int32_t)((uint32_t)(hi << TRACKER_FP_SHIFT) | (lo >> TRACKER_FP_SHIFT));
    smooth_dy_fp = s_old_y + s_new_y;

    // Convert back to integer
    int16_t out_dx = (int16_t)(smooth_dx_fp >> TRACKER_FP_SHIFT);
    int16_t out_dy = (int16_t)(smooth_dy_fp >> TRACKER_FP_SHIFT);

    // Clamp
    if (out_dx > MAX_MOUSE_DELTA) out_dx = MAX_MOUSE_DELTA;
    if (out_dx < -MAX_MOUSE_DELTA) out_dx = -MAX_MOUSE_DELTA;
    if (out_dy > MAX_MOUSE_DELTA) out_dy = MAX_MOUSE_DELTA;
    if (out_dy < -MAX_MOUSE_DELTA) out_dy = -MAX_MOUSE_DELTA;

    result->dx = out_dx;
    result->dy = out_dy;
    result->valid = (out_dx != 0 || out_dy != 0);

    stats.last_dx = out_dx;
    stats.last_dy = out_dy;
}
